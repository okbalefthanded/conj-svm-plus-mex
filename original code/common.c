...
void svm_predict_values(const svm_model *model, const svm_node *x, double *decf_values)
{
  // 2 3 4
...
void svm_destroy_model(svm_model *model)
{
  ...
  if (model->free_sv && model->l > 0)
  {
    ...
    // freeing the svm_plus model was commented by the a
    /*
      if(model->param.svm_type == SVM_PLUS)
